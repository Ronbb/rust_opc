...
```rust
use opc_classic_utils::{CoTaskMemArrayOut, CoTaskMemOut, NoCleanup, OwnedPwstr};

fn example() -> opc_classic_utils::Result<()> {
    let mut output = CoTaskMemArrayOut::new(3, NoCleanup);
    // unsafe { raw_com_method(output.as_mut_ptr()) };
    let values = unsafe { output.into_array() }?;
...
`server::ClassFactory` turns a Rust closure returning the project-owned
`ComObject` into a private COM class factory. `server::LocalClassRegistration`
registers it with COM and revokes the registration on drop. Its borrow of
`ComApartment` prevents the apartment from being uninitialized before the
registration is revoked. No Windows crate type appears in this public API.
